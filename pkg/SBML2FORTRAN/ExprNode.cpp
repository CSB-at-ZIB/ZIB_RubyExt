...
BinaryExpr::df(std::string const& n) const
{
    ...
    Expression mexpr = Expression(MINUS, _rexpr, 1.0);

    if ( (_op == '^') && (_rexpr.type() == 'L') ) 
    { 
        mexpr = Expression( _rexpr.eval(0) - 1.0 );
    }
...
BinaryExpr::prt(std::ostream& s) const
{
    if ( _op == '^' )
    {
        if ( (_lexpr.type() == 'L' || _lexpr.type() == 'V') && _rexpr.eq(1.0) )
        {
            s << _lexpr ;
        }
        else if ( _rexpr.eq(1.0) )
        {
            s << "(" << _lexpr << ")";
        }
        else
...
TernaryExpr::eval(Param& x) const
{
    ...
    switch(_op)
    {
        case TPLUS     : tmp = _lexpr.eval(x) + _mexpr.eval(x) + _rexpr.eval(x);
                         return tmp;
...
TernaryExpr::eval(double* y[]) const
{
    ...
    switch(_op)
    {
        case TPLUS     : tmp = _lexpr.eval(y) + _mexpr.eval(y) + _rexpr.eval(y);
                         return tmp;
...
TernaryExpr::df(std::string const& n) const
{
    ...
    switch (_op)
    {
        case TPLUS     : return Expression(TPLUS, _lexpr.df(n), _mexpr.df(n), _rexpr.df(n) );
        ...
        case HILLplus  : {
                             ...
                             Expression sump   = Expression(TPLUS, term1p, term2p, term3p);
